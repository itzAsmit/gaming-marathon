  ...
  return (
    <AdminLayout>
      <div className="p-4 md:p-8">
        <h1 className="text-2xl font-cinzel font-bold mb-1" style={{ color: "hsl(var(--brown-deep))", fontFamily: "'ROWAN', serif" }}>Hall of Fame</h1>
        ...
        <div className="space-y-8">
          {SEASONS.map((season) => (
            <div key={season} className="rounded-2xl p-4 md:p-6" style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--cream-dark))" }}>
              <h2 className="font-cinzel font-bold text-lg mb-4" style={{ color: "hsl(var(--brown-deep))", fontFamily: "'ROWAN', serif" }}>Season {season}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[1, 2, 3].map((rank) => {
                  const entry = getEntry(season, rank);
                  return (
                    <div key={rank}>
                      <label className="block text-xs font-cinzel tracking-widest mb-1.5" style={{ color: "hsl(var(--brown))", fontFamily: "'ROWAN', serif" }}>{RANK_LABELS[rank - 1]}</label>
                      <select
