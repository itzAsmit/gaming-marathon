  ...
  return (
    <section id="players" className="relative min-h-[100svh] md:min-h-screen py-24 px-4 scroll-mt-24 md:scroll-mt-28">
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          <div className="text-center py-20 font-cinzel text-sm tracking-widest" style={{ color: "hsl(var(--cream-dark) / 0.6)", fontFamily: "'ROWAN', serif" }}>
            LOADING PLAYERS...
          </div>
        ) : error ? (
          <div className="text-center py-20" style={{ color: "hsl(var(--cream-dark) / 0.8)", fontFamily: "'ROWAN', serif" }}>
                ...
                >
                  <div className="aspect-[3/4] overflow-hidden relative">
                    {toMediaSrc(player.portrait_url) ? (
                      ...
                      >
                        <span className="text-4xl font-cinzel font-bold" style={{ color: player.is_active ? "hsl(var(--gold))" : "#888", fontFamily: "'ROWAN', serif" }}>
                    ...
                    <div className="absolute top-2 right-2">
                      <span
                        ...
                        style={{
                          background: player.is_active ? "rgba(34, 197, 94, 0.25)" : "rgba(100, 100, 100, 0.25)",
                          color: "white",
                      ...
                      >
                        <span 
                          ...
                          style={{
                            background: "white",
                            boxShadow: player.is_active ? "0 0 6px rgba(255, 255, 255, 0.8)" : "none",
                          }}
                        />
                    ...
                    >
                      <span className="text-xs font-cinzel tracking-widest" style={{ color: "hsl(var(--gold))", fontFamily: "'ROWAN', serif" }}>
                        VIEW PROFILE →
                      </span>
                    </div>
                  </div>
                  <div className="p-3 text-center">
                    <p className="text-sm font-cinzel font-semibold truncate" style={{ color: player.is_active ? "hsl(var(--cream))" : "#888", fontFamily: "'ROWAN', serif" }}>
                      {player.name}
                    </p>
      ...
      <AnimatePresence>
        {selected && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            style={{ background: "hsla(var(--brown-deep) / 0.5)", backdropFilter: "blur(12px)" }}
              ...
              <div className="max-h-[calc(100dvh-6rem)] overflow-y-auto overflow-x-hidden md:max-h-none md:h-full md:overflow-hidden">
                ...
                <div className="relative md:h-full">
                  {toMediaSrc(selected.portrait_url) ? (
                    ...
                    >
                      <span className="text-7xl font-cinzel font-bold" style={{ color: selected.is_active ? "hsl(var(--gold))" : "#888", fontFamily: "'ROWAN', serif" }}>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4" style={{ background: "linear-gradient(to top, hsla(var(--brown-deep) / 0.95), transparent)" }}>
                    <div className="flex items-center gap-2 mb-1">
                      <p className="text-xs tracking-widest font-cinzel" style={{ color: "hsl(var(--gold))", fontFamily: "'ROWAN', serif" }}>
                        {selected.player_id}
                      </p>
                      <span
                        className="px-2.5 py-0.5 rounded-full text-[9px] font-semibold tracking-wide flex items-center gap-1.5"
                        style={{
                          background: selected.is_active ? "rgba(34, 197, 94, 0.25)" : "rgba(100, 100, 100, 0.25)",
                          color: "white",
                          backdropFilter: "blur(12px)",
                      ...
                      >
                        <span 
                          ...
                          style={{
                            background: "white",
                            boxShadow: selected.is_active ? "0 0 6px rgba(255, 255, 255, 0.8)" : "none",
                          }}
                        />
                ...
                <div className="relative p-5 md:p-3 lg:p-5 space-y-4 md:space-y-2 lg:space-y-4 md:h-full md:flex md:flex-col md:justify-start">
                  ...
                  {selected.bio && (
                    <div>
                      <p className="text-xs font-cinzel tracking-widest mb-2" style={{ color: "hsl(var(--gold))", fontFamily: "'ROWAN', serif" }}>BIO</p>
                  ...
                  {/* Stats */}
                  <div>
                    <p className="text-xs font-cinzel tracking-widest mb-3" style={{ color: "hsl(var(--gold))", fontFamily: "'ROWAN', serif" }}>STATS</p>
                    <div className="grid grid-cols-2 gap-2 md:gap-1 lg:gap-2">
                      {[
                        ...
                        <div key={s.label} className="rounded-xl p-2 md:p-1.5 lg:p-2.5 text-center" style={{ background: "hsla(var(--gold) / 0.08)", border: "1px solid hsla(var(--gold) / 0.2)" }}>
                          <p className="text-lg md:text-sm lg:text-lg font-cinzel font-bold" style={{ color: "hsl(var(--gold))", fontFamily: "'ROWAN', serif" }}>{s.value}</p>
                          <p className="text-xs md:text-[10px] font-cinzel tracking-wider" style={{ color: "hsl(var(--cream-dark) / 0.7)", fontFamily: "'ROWAN', serif" }}>{s.label}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Items */}
                  <div>
                    <p className="text-xs font-cinzel tracking-widest mb-2" style={{ color: "hsl(var(--gold))", fontFamily: "'ROWAN', serif" }}>ITEMS HOLDING</p>
                    <div className="flex flex-wrap gap-2">
                      {selected.items && selected.items.length > 0 ? (
                        selected.items.map((pi: any) => (
                          <span key={pi.items.name} className="px-3 py-1 md:px-2 md:py-0.5 rounded-full text-xs md:text-[10px] font-cinzel" style={{ background: "hsla(var(--gold) / 0.15)", color: "hsl(var(--gold))", border: "1px solid hsla(var(--gold) / 0.3)", fontFamily: "'ROWAN', serif" }}>
                            {pi.items.name}
                          </span>
                        ))
                      ) : (
                        <span className="text-xs font-cinzel" style={{ color: "hsl(var(--cream-dark) / 0.75)", fontFamily: "'ROWAN', serif" }}>
                          NONE
                        </span>
                      )}
                    </div>
                  ...
                  {selected.proficiencies && selected.proficiencies.length > 0 && (
                    <div>
                      <p className="text-xs font-cinzel tracking-widest mb-3 md:mb-2" style={{ color: "hsl(var(--gold))", fontFamily: "'ROWAN', serif" }}>GAME PROFICIENCY</p>
